...
use anyedge_adapter_cloudflare::{
...
fn build_test_app() -> App {
    async fn capture_uri(ctx: RequestContext) -> Result<anyedge_core::Response, EdgeError> {
    ...
    async fn mirror_body(ctx: RequestContext) -> Result<anyedge_core::Response, EdgeError> {
    ...
    async fn stream_response(_ctx: RequestContext) -> Result<anyedge_core::Response, EdgeError> {
...
fn cf_request(method: CfMethod, path: &str, body: Option<&[u8]>) -> CfRequest {
    ...
    headers.set("x-anyedge-test", "1").expect("custom header");
...
async fn from_core_response_translates_status_headers_and_streaming_body() {
    ...
    let header = cf_response.headers().get("x-anyedge-res").unwrap();
