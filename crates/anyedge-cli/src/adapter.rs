use anyedge_adapter::{self as adapter_registry, AdapterAction};
...
pub fn execute(
    ...
    let adapter = adapter_registry::get_adapter(adapter_name).ok_or_else(|| {
        ...
        if available.is_empty() {
            if manifest.is_none() {
                format!(
                    "adapter `{}` is not registered in this build. Provide an `anyedge.toml` (or set `ANYEDGE_MANIFEST`) so the CLI can load adapters, or rebuild `anyedge-cli` with the `{adapter_name}` adapter feature enabled.",
                    adapter_name
                )
            } else {
                format!(
...
fn run_shell(
    ...
    println!(
        "[anyedge] executing `{}` for adapter `{}` in {}",
        full_command,
        adapter_name,
        cwd.display()
    );
