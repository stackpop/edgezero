...
impl SpinKvStore {
    ...
    ///
    /// # Errors
    /// Returns [`KvError::Internal`] if the named store cannot be opened.
    #[inline]
    ...
    ///
    /// # Errors
    /// Returns [`KvError::Internal`] if the default-labelled store cannot
    /// be opened.
    #[inline]
    pub fn open_default() -> Result<Self, KvError> {
        Self::open(DEFAULT_KV_STORE_NAME)
    }
}
