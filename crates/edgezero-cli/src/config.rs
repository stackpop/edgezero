...
/// Reject `..` traversal and absolute paths (and, under `--local`, paths
/// that escape the adapter crate dir) in EVERY declared adapter's
/// manifest / crate strings, BEFORE `run_shared_checks` dispatches
/// per-adapter validation that joins those strings onto `manifest_root`
/// (e.g. Spin's `fs::read_to_string(manifest_root.join(manifest))`). The
/// absolute/`..` rejection applies in every config-push path (cloud still
/// joins the path for the vendor CLI / service-id lookup); the stronger
/// "manifest sits inside the crate dir" containment is `--local`-only,
/// since cloud fixtures legitimately keep e.g. `wrangler.toml` at the
/// project root and vendor dispatch creates no files. Every adapter is
/// checked because shared checks iterate every adapter, not just the
/// targeted one.
fn assert_push_adapter_paths(validation: &ValidationContext, local: bool) -> Result<(), String> {
    let manifest_root_for_check = validation
        .manifest_path()
        .parent()
        .filter(|parent| !parent.as_os_str().is_empty())
        .unwrap_or_else(|| Path::new("."));
    for adapter_cfg in validation.manifest().adapters.values() {
        if local {
    ...
    C: DeserializeOwned + Serialize + Validate + AppConfigMeta,
{
    ...
    assert_push_adapter_paths(&ctx.validation, args.local)?;
...
[adapters.fastly.adapter]
crate = "crates/demo-app-adapter-fastly"
manifest = "crates/demo-app-adapter-fastly/fastly.toml"
...
"#;
        ...
        // A corrupt-but-PARSING prior STATE must let a local dry-run reach the
        // writer's degrading orphan count rather than hitting the Spin-Cloud
        // rejection: a corrupt-but-parsing pointer, and a non-string root. Both
        // keep a valid `[local_server.config_stores.app_config.contents]` table
        // (the store IS provisioned) with unusable CONTENT.
        //
        // A wholly MALFORMED `fastly.toml` (unparseable) is deliberately NOT in
        // this set: the local dry-run models the real push, which refuses when
        // the manifest can't be parsed -- covered by
        // `local_real_push_over_malformed_toml_still_fails`.
        ...
        for (label, fastly_toml) in cases {
            ...
            // The manifest must resolve INSIDE the adapter crate dir (the
            // strict-local containment rule), so the local `fastly.toml` lives
            // under `crates/demo-app-adapter-fastly/`, not the project root.
            let fastly_path = dir
                .path()
                .join("crates/demo-app-adapter-fastly/fastly.toml");
            fs::create_dir_all(fastly_path.parent().expect("crate dir parent"))
                .expect("create adapter crate dir");
            fs::write(&fastly_path, &fastly_toml).expect("write fastly.toml");
...
"#;
        let _lock = manifest_guard().lock().expect("manifest guard");
        let (dir, manifest, _) = setup_project(FASTLY_ONLY_MANIFEST, FIXTURE_APP_CONFIG);
        // Manifest must resolve inside the adapter crate dir (strict-local
        // containment), so write the local `fastly.toml` there, not at root.
        let fastly_path = dir
            .path()
            .join("crates/demo-app-adapter-fastly/fastly.toml");
        fs::create_dir_all(fastly_path.parent().expect("crate dir parent"))
            .expect("create adapter crate dir");
        fs::write(&fastly_path, "this is not = = valid toml").expect("write fastly.toml");
