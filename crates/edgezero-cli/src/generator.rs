...
mod tests {
    ...
    fn assert_scaffold_workspace(project_dir: &Path) {
        ...
        // per-machine ids / operator-set defaults. All four adapter
        // manifests are now provision-generated; `axum.toml` joined
        // the list when Axum's `synthesise_baseline_manifest` was
        // wired up (2026-07 refactor).
        for entry in [
            "axum.toml",
            "fastly.toml",
            "spin.toml",
