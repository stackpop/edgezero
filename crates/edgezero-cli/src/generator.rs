...
mod tests {
    ...
    fn assert_scaffold_workspace(project_dir: &Path) {
        ...
        // per-machine ids. axum.toml is intentionally excluded from
        // this list — Axum owns its manifest and it stays tracked.
        for entry in [
            "fastly.toml",
            "spin.toml",
            "wrangler.toml",
        ...
        // `axum.toml` MAY appear in explanatory comments but must not
        // survive as an ignore rule (a bare or globbed line). Match
        // non-comment lines only.
        let axum_toml_active_rule = gitignore.lines().any(|line| {
            let trimmed = line.trim();
            !trimmed.starts_with('#') && trimmed.contains("axum.toml")
        });
        assert!(
            !axum_toml_active_rule,
            "axum.toml must NOT be gitignored (Axum owns its manifest): {gitignore}"
        );
