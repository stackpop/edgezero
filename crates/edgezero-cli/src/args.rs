...
/// changes. Total bool count is 6 (`dry_run`, `local`, `no_diff`,
/// `no_env`, `staging`, `yes`), which trips
...
#[expect(
    clippy::struct_excessive_bools,
    reason = "6 bools by design: source-compat for downstream CLIs that assign \
              `args.no_diff` / `args.no_env` directly. Each is a distinct CLI flag \
              (dry_run, local, no_diff, no_env, staging, yes); grouping into a \
              sub-struct trips E0609 at every existing call site."
)]
#[derive(clap::Args, Debug)]
#[non_exhaustive]
