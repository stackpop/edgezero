...
#[cfg(feature = "fastly")]
impl From<anyedge_core::manifest::ResolvedLoggingConfig> for FastlyLogging {
    fn from(config: anyedge_core::manifest::ResolvedLoggingConfig) -> Self {
        Self {
...
#[cfg(feature = "fastly")]
pub fn run_app<A: anyedge_core::app::Hooks>(
    manifest_src: &str,
    req: fastly::Request,
) -> Result<fastly::Response, fastly::Error> {
    let manifest_loader = anyedge_core::manifest::ManifestLoader::load_from_str(manifest_src);
    let logging = manifest_loader.manifest().logging_or_default("fastly");
    run_app_with_logging::<A>(logging.into(), req)
}

#[cfg(feature = "fastly")]
pub fn run_app_with_logging<A: anyedge_core::app::Hooks>(
...
mod tests {
    ...
    fn fastly_logging_from_manifest_converts_defaults() {
        let config = anyedge_core::manifest::ResolvedLoggingConfig {
            endpoint: Some("endpoint".to_string()),
            echo_stdout: Some(false),
            level: anyedge_core::manifest::LogLevel::Debug,
        };
