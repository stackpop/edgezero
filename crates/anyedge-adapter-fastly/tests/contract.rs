...
use anyedge_core::http::{response_builder, Method, Response, StatusCode};
...
fn fastly_request(method: FastlyMethod, path: &str, body: Option<&[u8]>) -> FastlyRequest {
    ...
    req.set_header("x-anyedge-test", "1");
...
fn from_core_response_translates_status_headers_and_streaming_body() {
    ...
    assert!(fastly_response.get_header("x-anyedge-res").is_some());
