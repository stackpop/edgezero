...
tar -xOzf "$archive" release.json | jq -e \
  '.format == 1 and .lifecycle_protocol == 1 and .adapter == "fastly"
   and (.manifests.adapter.path == "fastly.toml")' >/dev/null
tar -tzf "$archive" | grep -qx 'fastly.toml'
if tar -tzf "$archive" | grep -qx 'adapters/spin/spin.toml'; then
  printf 'packager included an unselected Spin manifest\n' >&2
  exit 1
fi
...
mv "$WORK_DIR/workspace/fastly.toml" "$WORK_DIR/workspace/fastly.real.toml"
ln -s fastly.real.toml "$WORK_DIR/workspace/fastly.toml"
if GITHUB_WORKSPACE="$WORK_DIR/workspace" \
  ...
  printf 'packager accepted a symlinked Fastly adapter manifest\n' >&2
  exit 1
fi
grep -Fq "adapter-manifest must be a regular file" "$WORK_DIR/symlink.log"
rm "$WORK_DIR/workspace/fastly.toml"
mv "$WORK_DIR/workspace/fastly.real.toml" "$WORK_DIR/workspace/fastly.toml"
