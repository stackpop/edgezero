...
boot_runtime() {
  ...
  case "$adapter" in
    ...
    fastly)
      ...
      # by seed_secret_for_adapter. Current Viceroy uses `serve`
...
for suite in "${SUITES[@]}"; do
  ...
  # Spin compatibility pre-check: spin-sdk 7 imports stable
  # wasi:http/types@0.3.0, which Spin < 4.0 does not provide.
  # Auto-skip the row with a clear note rather than fail in the
  # wasm linker; operators can install Spin 4.x
  # and re-run.
  if [ "$adapter" = "spin" ]; then
    ...
    if [ -n "$spin_minor" ] && [ "$spin_minor" -lt 400 ]; then
      printf '\n=== 12.7 __KEY override smoke: spin SKIPPED (CLI %s; need >= 4.0 for spin-sdk 7 wasi:http 0.3) ===\n' "$spin_ver"
      continue
    fi
  fi
  ...
  case "$adapter" in
    axum)
      ...
      ;;
    cloudflare)
      rm -rf "$DEMO_DIR/crates/app-demo-adapter-cloudflare/.wrangler"
      ;;
    spin)
