...
- [x] CLI: manifest-driven `anyedge build --adapter fastly|cloudflare|axum` wiring via `crates/anyedge-cli/src/main.rs` + adapter manifests.
...
- [ ] CLI
  ...
  - [x] `anyedge build --adapter fastly|cloudflare`
  - [x] `anyedge deploy --adapter fastly|cloudflare`
  - [ ] `anyedge dev` improvements: better HTTP parsing, hot-reload
- [x] Config
  - [x] `anyedge.toml`: app name, routes, provider-specific settings
