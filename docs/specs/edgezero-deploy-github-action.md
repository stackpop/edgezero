...
| `provider-env-clear` | No       | shipped-adapter aliases | The DYNAMIC half of a two-layer scrub keeping provider credentials out of `build-app-cli`'s app-controlled commands (`cargo metadata`, `cargo build`, the built CLI's `--help`). **Static half:** the step's own `env:` blanks the shipped adapters' aliases (Fastly/Cloudflare/Spin) outright — so the step process never held them, and this is the only layer that also covers the upload step. **Dynamic half (this input):** a JSON array naming a provider this action cannot know about; the script re-execs itself with those removed (`unset` alone leaves them readable via `/proc/<pid>/environ`) and the `run:` body `exec`s the script so no ancestor shell keeps a copy. The default repeats the shipped aliases so the dynamic layer is self-contained. Rejected unless it is a JSON array of non-empty identifier strings (validated in `jq` on the decoded values, so no control character survives) — a permissive parse would silently scrub nothing. |
...
| `rust-toolchain`    | No       | `auto`        | Application Rust toolchain for the deploy build; `auto` follows the §7 precedence. Forwarded to the engine.                                                                  |
