...
- [ ] Pin root and app-demo `fastly` requirements to exact `=0.13.1`; refresh both locks with `cargo update --offline -p fastly --precise 0.13.1` and `cargo update --offline --manifest-path examples/app-demo/Cargo.toml -p fastly --precise 0.13.1`. Assert both `cargo tree --offline --locked` graphs resolve exactly `fastly v0.13.1` before writing exhaustive SDK mappings.
...
- [ ] Add guard-order tests before cold batch registration and immediately before dispatch: absolute expiry -> attributed 504; live-deadline setup over `BATCH_DISPATCH_SLACK_MAX` -> internal 500; no deadline uses the default budget without the live-deadline slack failure. Prove an over-slack later cache miss invokes no additional registration.
...
- [ ] Implement `BackendIdentity(scheme, host_name, resolved_port, tls_mode, budget_ms)` and `ez_` names. Use true ceil-to-millisecond conversion, including a 1 ms + 1 ns -> 2 ms boundary case. Cache name -> `(identity, backend)` behind `Mutex`; never hold the lock across `finish()`. Leave SDK connection pooling at its default: Fastly reuses only same-name, exact-same-settings backends, and the identity/settings already separate budgets and TLS modes. Probe multi-host heterogeneous budgets because each distinct tuple may require synchronous registration.
