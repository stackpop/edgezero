...
- [ ] Add failing cases for empty batch, complete preflight and index alignment, every valid dispatch before first wait, one-slot `send_all`/`send` equivalence, reverse completion with stable order, partial dispatch/wait/body errors, preservation of pre-existing `Done` errors during poll sweeps, one method-entry `batch_now`, and unresolved-slot internal error. Advance the clock during normalization/preflight/backend preparation and prove the original `send`/`send_all` snapshot owns the budget. Include GET/HEAD body errors beating batch-only errors and no polls of rejected source streams.
...
- [ ] Add identity/compressed `Content-Length` cases proving malformed/conflicting values and encoded/effective-identity decoded overages reject before the first Fastly body read; include exactly one bare `Content-Encoding: identity`. Compressed and unknown/stacked passthrough lengths are never compared with the decoded cap.
...
- [ ] Map configured phase timeout -> attributed 504; early unconfigured DNS timeout ->
  ...
  transport/protocol -> typed 502; local invariant/platform internal -> 500; unspecified
...
- [ ] Add table-driven capability tests and publish Fastly's override only now: Native for header fidelity; BestEffort for outbound HTTP, deadlines, flexible phase budget, slot isolation, streamed-upload deadlines, and lazy response passthrough; future capabilities -> Unsupported.
