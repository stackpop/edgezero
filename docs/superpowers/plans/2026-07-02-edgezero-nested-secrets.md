...
- **`field_has_nested_optin` false-positive on `#[app_config()]`:** the CI-guard helper checked only `parse_nested_meta(..).is_ok()`, so a bare `#[app_config()]` (no `nested`) reported opt-in. Now tracks a `found` flag and returns `parsed.is_ok() && found` (Task 5 Step 3).
  - **Round-5 correction:** the *derive*'s `nested_optin` had the mirror bug (returned `Ok(false)` for empty `#[app_config()]`, silently NOT recursing and dropping the child's secrets — the opposite failure from the guard's false-positive). Fixed to hard-error when an `app_config` attribute is present without `nested`, per its documented contract; added the `app_config_empty.rs` trybuild fixture. (An earlier note here wrongly claimed the derive already handled this.)
