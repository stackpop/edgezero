...
- [ ] Run `cargo check -p edgezero-core`; expect success.
- [ ] Run `cargo tree --locked -p edgezero-core -e normal | rg 'url v2\.5\.8'`; expect exactly the pinned normal dependency.
...
- [ ] Add failing `outbound_request_*` tests for defaults, every builder, `into_parts`/`from_parts`, JSON content type, and header byte validation.
...
- [ ] Add rejection rows for non-HTTP schemes, relative/no-authority targets, userinfo, raw `#` fragments, malformed UTF-8/header bytes, and invalid Brotli-window ranges. Add a positive row proving percent-encoded `%23` remains path/query data rather than being rejected as a fragment.
...
- [ ] Implement raw-string construction in this order: reject a literal `#`; `Url::parse`; reject scheme/authority/userinfo violations; clear default ports; serialize once; parse that serialization into `http::Uri`. `new(Uri)` runs the same post-parse validation but cannot recover a fragment discarded before it received the typed URI.
...
- [ ] Add failing `dispatch_validation_*` tests for the portable method set, GET/HEAD body rules, streamed-body detection, streamed-response detection, and invalid policy combinations.
