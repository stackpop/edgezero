...
- [ ] Through the target-neutral `NativeBody`/header seam, cover every content-encoding row, effective-identity and raw-passthrough pre-poll rejection including explicit `identity`, unequal encoded/decoded/final caps, Brotli decoder-state charge, cumulative guest-visible header sections, repeated `Set-Cookie`, 205 settlement, first-chunk-before-EOF, typed completion, and decoder/cap/deadline ownership. These tests own orchestration; Task 7 repeats Workers-only SDK response boundaries under workerd.
