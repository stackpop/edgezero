...
| File | What it holds |
| --- | --- |
| `crates/edgezero-adapter-fastly/src/chunked_config.rs` | `prior_chunk_keys` (validated, canonical, prefix-scoped), `validate_pointer_chunks` (internally consistent; positions not keys in diagnostics; bounded, checked lengths), `gc_classify_root` (fail-closed GC classifier), `gc_verify_generation` (content-address check — NOT proof of authorship), `redact_json_err`, `chunk_key_parts` / `chunk_key_generation` (canonical-only, index must fit `usize`), unit tests |
...
| `crates/edgezero-adapter-fastly/src/cli.rs` (config gc) | `gc_config_entries` → `gc_fastly_config_store` (rejects a zero window at the destructive boundary; deletes whole generations, stopping each at its first failure) → `plan_gc_reclamation` (groups candidates by generation) → `prove_generation` (round-trips against `prepare_fastly_config_entries`), `assemble_pointer_chunks`; `list_config_store_entries` (bare-array-only, non-empty fields, no duplicate keys), `chunk_key_generation_any`, `parse_rfc3339_secs`, `unix_now_secs`, `delete_config_store_entry` (`--key --auto-yes`, never `--all`; every failure is a failure); `redact_describe_response` / `redact_stderr` |
