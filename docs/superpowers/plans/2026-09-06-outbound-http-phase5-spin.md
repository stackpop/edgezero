...
- [ ] Use `join_all` for eligible complete batch slots after full preflight. Construct each `OutboundSlotResult` when its own preflight/exchange becomes terminal; vector assembly must not sample time.
...
- [ ] Add table-driven capability tests and publish Spin's override only now: Native for outbound HTTP, header fidelity, and slot isolation; BestEffort for deadlines, flexible phase budget, streamed-upload deadlines, and lazy response passthrough; Unsupported for complete resource accounting; future capabilities -> Unsupported.
