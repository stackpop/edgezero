...
- `--staging` — write the `<logical-store-id>_staging` variant in the SAME store,
  ...
  `edgezero_runtime_env` link at this key via the service-scoped
  `EDGEZERO__SERVICES__<SERVICE_ID>__STORES__CONFIG__<ID>__KEY` entry in its
...
`config push`, this is **typed-only** — the bundled `edgezero` binary
...
| `fastly`     | For each KV / config / secret id: shells out to `fastly <kind>-store create --name=<platform-name>` (using the same `<platform-name>` resolution), then appends the `[setup.<kind>_stores.<platform-name>]` table to `fastly.toml`. Provision writes ONLY `[setup.*]` (the remote/deploy half); the `[local_server.*]` seeding is written by `config push --local` (config stores only). If the setup table is already present, resource creation and manifest editing are skipped. A live run upserts explicit non-default logical-to-physical name mappings in `edgezero_runtime_env` under `EDGEZERO__SERVICES__<SERVICE_ID>__*`. It never infers a mapping deletion from an absent or default-valued override. Non-default mappings require top-level `service_id` in `fastly.toml` or `FASTLY_SERVICE_ID`. Store IDs are not persisted — `config push` resolves them on demand. |
...
whose `[setup.<kind>_stores.<id>]` block already exists, then reads
`edgezero_runtime_env` and upserts service-scoped mappings for explicit
non-default store-name overrides. It does not delete a mapping when its override
is absent, returns to the logical default, or is no longer declared: absence
could mean a lost production environment value rather than operator intent. It
also does not delete legacy unscoped keys, another service's namespace, or
unrelated runtime entries; the runtime ignores unscoped keys, which must be
migrated manually. If non-default mappings
are requested, top-level `service_id` in `fastly.toml` or `FASTLY_SERVICE_ID`
must identify the owning service before provision performs any remote mutation.

To return one store to its logical default, first upgrade every CI job and
operator that can run `provision`, remove the override from those environments,
then explicitly delete only that service-scoped mapping:
...
Use `CONFIG`, `KV`, or `SECRETS` for `<KIND>`. Provisioning binaries predating
the upsert-only behavior can interpret a missing override as deletion, so do not
remove overrides during a mixed-version rollout.
...
Auth commands run the native CLI attached to the terminal. A missing binary
surfaces with an install hint; on a non-zero exit the child's stderr remains
visible and EdgeZero reports its exit status. This differs from Fastly helpers
that capture potentially credential-bearing arguments or output: those failure
diagnostics are suppressed rather than echoed.
