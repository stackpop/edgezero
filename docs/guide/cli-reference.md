...
| `spin`       | HTTP POST to the seed handler at `/__edgezero/config/seed` on the running Spin app. Resolves the seed URL from `--seed-url`, then `EDGEZERO__ADAPTERS__SPIN__SEED_URL` (or `…__LOCAL_SEED_URL` under `--local`), then `[adapters.spin.commands].seed_url` in `edgezero.toml`. Auth token resolved from `--seed-token` or `EDGEZERO__ADAPTERS__SPIN__SEED_TOKEN` (never read from `edgezero.toml`). The handler validates the token against the server-side value (constant-time, 16-byte floor — see Stage 3 docs) and writes entries verbatim into the `spin_sdk::key_value::Store` whose label matches the resolved platform name of the target `[stores.config]` id. Keys pass through with no translation. Secret variables stay manual in `spin.toml` (`[variables]` + `secret = true`) — `config push` skips `SECRET_FIELDS`. |
