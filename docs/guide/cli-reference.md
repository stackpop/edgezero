...
| `spin`       | **Under restructure.** The previous seed-handler HTTP POST was retired in commit `<this>` because it exposed a permanent EdgeZero-owned attack surface inside every deployed app. The per-backend replacement (SQLite-direct writes for `type = "spin"` local dev; `spin cloud key-value set` shellouts for Fermyon Cloud deployments) lands in the next commit on the `feature/extensible-cli` branch. Until then `config push --adapter spin` returns a clear "under restructure" error. See `docs/superpowers/plans/2026-06-04-spin-per-backend-push.md`. |
