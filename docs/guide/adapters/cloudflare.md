...
`run_app` reads config-store metadata generated by `edgezero_core::app!` and injects the configured
Cloudflare binding automatically. If you implement `Hooks` manually and need runtime manifest
fallbacks, use `run_app_with_manifest`.
