...
This is the exact baseline `provision --local` synthesises. EdgeZero
drives the build through `edgezero build --adapter cloudflare` (which
runs `cargo build` + the artifact copy), so no `[build]` block is
emitted; add one yourself only if you invoke `wrangler` directly.
...
`.dev.vars`. Teammates regenerate it after cloning with:
