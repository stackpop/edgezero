...
`[adapters.cloudflare.adapter].crate` rename). EdgeZero drives the
build through `edgezero build --adapter cloudflare` (which runs
`cargo build` + the artifact copy), so no `[build]` block is emitted;
add one yourself only if you invoke `wrangler` directly.
