...
name = "my-app-adapter-cloudflare"
...
it `<app>-adapter-cloudflare`, honouring any
`[adapters.cloudflare.adapter].crate` rename). The `[build]` command is
**required**: `main` points at the `worker-build`-generated
`build/worker/shim.mjs`, so without it `edgezero serve` / `edgezero deploy`
(which shell out to `wrangler dev` / `wrangler deploy` from the crate root)
would have no shim to load. `wrangler` runs the command for you before
dev/deploy.

::: tip Cloudflare / Fastly / Spin manifests are gitignored
`wrangler.toml` is generated by the scaffolder (`edgezero new` runs
`provision --adapter cloudflare --local` for you) and is **not
committed** — the workspace `.gitignore` excludes it, along with
`.dev.vars`. Teammates regenerate it after cloning with (the project
CLI isn't installed on a fresh clone, so build-and-run it from source):

```bash
cargo run -p my-app-cli -- provision --adapter cloudflare --local
```

`provision --local` synthesises `wrangler.toml` if it's missing and
...
`provision_typed` additionally writes `.dev.vars`
placeholders — one line per `#[secret]` field on your typed
`MyAppConfig` — so `wrangler dev` resolves them locally. Fill in
real values before running the dev server; `.dev.vars` is
gitignored so the placeholders stay per-developer.
:::
